      ...
      <div className="flex-1 flex gap-8 w-full min-h-0 overflow-hidden">
        ...
        <main className="flex-1 flex flex-col min-h-0 relative h-full pr-2">
            <div className="flex-1 flex flex-col gap-6 min-h-0">
                ...
                {/* Charts Section */}
                {(activeMenu === 'overview' || activeMenu === 'charts') && (
                    <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm flex-1 flex flex-col min-h-0">
                        <div className="flex items-center justify-between mb-6 pb-4 border-b border-slate-100 shrink-0">
                            <div>
                                <h3 className="text-lg font-bold text-slate-800">Clinical Charts</h3>
                    ...
                    <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm flex-1 flex flex-col min-h-0">
                        ...
                        {loadingImages ? (
                            <div className="flex-1 flex flex-col items-center justify-center py-12">
                                ...
                                <p className="text-slate-400 text-sm">Retrieving images...</p>
                            </div>
                        ) : images.length === 0 ? (
                            <div className="bg-slate-50 border border-slate-200 border-dashed rounded-2xl p-10 flex flex-col items-center justify-center text-center flex-1">
                                <ImageIcon className="w-10 h-10 text-slate-300 mb-3" />
                                <h4 className="text-base font-semibold text-slate-700">No radiographs found</h4>
                                <p className="text-slate-500 text-sm mt-1">There are no radiography scans linked to this patient's visits.</p>
                            </div>
                        ) : (
                            <div className="flex-1 overflow-y-auto pr-1 custom-scrollbar">
                                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                                    {images.map((img) => (
                                        <div 
                                            key={img.image_id} 
                                            className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden group flex flex-col cursor-pointer"
                                            onClick={() => handleImageClick(img.chart_id, img.image_id)}
                                        >
                                            <div className="aspect-square relative bg-slate-950 flex items-center justify-center overflow-hidden">
                                                {img.image_url ? (
                                                    <img 
                                                        src={img.image_url} 
                                                        alt={img.description || img.image_file}
                                                        className="w-full h-full object-cover transition-transform duration-300 group-hover:scale-105"
                                                    />
                                                ) : (
                                                    <span className="text-slate-600 font-mono text-[10px]">No Preview</span>
                                                )}
                                                <div className="absolute inset-0 bg-slate-950/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                                                    <span className="text-white text-xs font-bold border border-white/30 bg-slate-900/80 px-3 py-1 rounded-full backdrop-blur-sm">
                                                        View Chart Detail
                                                    </span>
                                                </div>
                                            </div>
                                            <div className="p-3 flex-1 flex flex-col">
                                                <span className="px-1.5 py-0.5 text-[9px] font-bold uppercase tracking-wider rounded bg-teal-50 text-teal-700 border border-teal-100 w-fit">
                                                    {img.image_type}
                                                </span>
                                                <p className="text-xs font-semibold text-slate-700 mt-2 line-clamp-2 flex-1">
                                                    {img.description || "No description."}
                                                </p>
                                                <span className="text-[9px] text-slate-400 font-mono block mt-2 font-medium">
